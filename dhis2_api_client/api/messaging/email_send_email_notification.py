from http import HTTPStatus
from typing import Any, Optional, Union

import httpx

from ... import errors
from ...client import AuthenticatedClient, Client
from ...models.web_message import WebMessage
from ...types import UNSET, Response, Unset


def _get_kwargs(
    *,
    message: str,
    recipients: list[str],
    subject: Union[Unset, str] = "DHIS 2",
) -> dict[str, Any]:
    params: dict[str, Any] = {}

    params["message"] = message

    json_recipients = recipients

    params["recipients"] = json_recipients

    params["subject"] = subject

    params = {k: v for k, v in params.items() if v is not UNSET and v is not None}

    _kwargs: dict[str, Any] = {
        "method": "post",
        "url": "/email/notification#sendEmailNotification",
        "params": params,
    }

    return _kwargs


def _parse_response(*, client: Union[AuthenticatedClient, Client], response: httpx.Response) -> Optional[WebMessage]:
    if response.status_code == 200:
        response_200 = WebMessage.from_dict(response.json())

        return response_200
    if client.raise_on_unexpected_status:
        raise errors.UnexpectedStatus(response.status_code, response.content)
    else:
        return None


def _build_response(*, client: Union[AuthenticatedClient, Client], response: httpx.Response) -> Response[WebMessage]:
    return Response(
        status_code=HTTPStatus(response.status_code),
        content=response.content,
        headers=response.headers,
        parsed=_parse_response(client=client, response=response),
    )


def sync_detailed(
    *,
    client: Union[AuthenticatedClient, Client],
    message: str,
    recipients: list[str],
    subject: Union[Unset, str] = "DHIS 2",
) -> Response[WebMessage]:
    """[no description yet]

    Args:
        message (str):
        recipients (list[str]):
        subject (Union[Unset, str]):  Default: 'DHIS 2'.

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Response[WebMessage]
    """

    kwargs = _get_kwargs(
        message=message,
        recipients=recipients,
        subject=subject,
    )

    response = client.get_httpx_client().request(
        **kwargs,
    )

    return _build_response(client=client, response=response)


def sync(
    *,
    client: Union[AuthenticatedClient, Client],
    message: str,
    recipients: list[str],
    subject: Union[Unset, str] = "DHIS 2",
) -> Optional[WebMessage]:
    """[no description yet]

    Args:
        message (str):
        recipients (list[str]):
        subject (Union[Unset, str]):  Default: 'DHIS 2'.

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        WebMessage
    """

    return sync_detailed(
        client=client,
        message=message,
        recipients=recipients,
        subject=subject,
    ).parsed


async def asyncio_detailed(
    *,
    client: Union[AuthenticatedClient, Client],
    message: str,
    recipients: list[str],
    subject: Union[Unset, str] = "DHIS 2",
) -> Response[WebMessage]:
    """[no description yet]

    Args:
        message (str):
        recipients (list[str]):
        subject (Union[Unset, str]):  Default: 'DHIS 2'.

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        Response[WebMessage]
    """

    kwargs = _get_kwargs(
        message=message,
        recipients=recipients,
        subject=subject,
    )

    response = await client.get_async_httpx_client().request(**kwargs)

    return _build_response(client=client, response=response)


async def asyncio(
    *,
    client: Union[AuthenticatedClient, Client],
    message: str,
    recipients: list[str],
    subject: Union[Unset, str] = "DHIS 2",
) -> Optional[WebMessage]:
    """[no description yet]

    Args:
        message (str):
        recipients (list[str]):
        subject (Union[Unset, str]):  Default: 'DHIS 2'.

    Raises:
        errors.UnexpectedStatus: If the server returns an undocumented status code and Client.raise_on_unexpected_status is True.
        httpx.TimeoutException: If the request takes longer than Client.timeout.

    Returns:
        WebMessage
    """

    return (
        await asyncio_detailed(
            client=client,
            message=message,
            recipients=recipients,
            subject=subject,
        )
    ).parsed
